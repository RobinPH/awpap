...
class EnsureCompleteProfile
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (! $request->user() ) {
            return redirect()->route("login")->with(["warning" => "You must be logged in to access that page."]);
        }

        $user = $request->user();

        if (!isset($user->first_name) || !isset($user->last_name) || !isset($user->phone) || !isset($user->birthdate) || !isset($user->civil_status)) {
            return redirect()->route("profile")->with(["warning" => "Complete your profile first to access that page."]);
        }
